...
PARAMETERS = {
    ...
    'FSBUPDATED': (
        19, 23, 'int', 1, 0, 'ro', 'FSB Update Decision.', '0 = false (FSB was not updated)',
...
class RespeakerAudio(object):
    # TODO: Find a way to increase the volume instead of reading the file again
    # TODO: Get the hotword state instead of using parameters
...
def on_audio(data, channel):
    ...
    if channel == 0:
        ...
        if sum(is_voice_buf) == 0:
            if len(audio_buf) > 0:
                if rospy.get_param('/flow_manager/notify_sound_playing'):
                    return
                ...
                audio_buf.insert(
                    0, np.zeros(int(silent_duration * respeaker_audio.rate)).tobytes()
                )
...
if __name__ == "__main__":
    ...
    silent_duration = rospy.get_param("silent_duration", 0.1)
